...
#define infect(otri) (otri).tri[6] = (triangle)((INT_PTR)(otri).tri[6] | (INT_PTR)2l)

#define uninfect(otri) (otri).tri[6] = (triangle)((INT_PTR)(otri).tri[6] & ~(INT_PTR)2l)
...

{
    ...
    pool->nextitem = (VOID*)(alignptr + (INT_PTR)pool->alignbytes - (alignptr % (INT_PTR)pool->alignbytes));
...

{
    ...
    pool->firstblock =
        (VOID**)trimalloc(pool->itemsfirstblock * pool->itembytes + (int)sizeof(VOID*) + pool->alignbytes);
    ...
    if (pool->deaditemstack != (VOID*)NULL) {
        ...
        if (pool->unallocateditems == 0) {
            ...
            if (*(pool->nowblock) == (VOID*)NULL) {
                ...
                newblock =
                    (VOID**)trimalloc(pool->itemsperblock * pool->itembytes + (int)sizeof(VOID*) + pool->alignbytes);
...

{
    ...
    pool->pathitem = (VOID*)(alignptr + (INT_PTR)pool->alignbytes - (alignptr % (INT_PTR)pool->alignbytes));
...

{
    ...
    m->dummytri =
        (triangle*)(alignptr + (INT_PTR)m->triangles.alignbytes - (alignptr % (INT_PTR)m->triangles.alignbytes));
    ...
    if (b->usesegments) {
        ...
        m->dummysub =
            (subseg*)(alignptr + (INT_PTR)m->subsegs.alignbytes - (alignptr % (INT_PTR)m->subsegs.alignbytes));
...

{
    ...
    poolinit(
        &m->vertices, vertexsize, VERTEXPERBLOCK, m->invertices > VERTEXPERBLOCK ? m->invertices : VERTEXPERBLOCK,
        sizeof(REAL)
    );
}
...

{
    ...
    poolinit(
        &m->triangles, trisize, TRIPERBLOCK,
...

{
    ...
    foundvertex = (char*)(alignptr + (INT_PTR)m->vertices.alignbytes - (alignptr % (INT_PTR)m->vertices.alignbytes));
...
#ifdef ANSI_DECLARATORS
    REAL
    incircle(struct mesh* m, struct behavior* b, vertex pa, vertex pb, vertex pc, vertex pd)
#else  /* not ANSI_DECLARATORS */
    REAL
...
static
    ...
    REAL
    orient3dadapt(
        vertex pa, vertex pb, vertex pc, vertex pd, REAL aheight, REAL bheight, REAL cheight, REAL dheight,
        REAL permanent
    )
#else  /* not ANSI_DECLARATORS */
    REAL
    orient3dadapt(pa, pb, pc, pd, aheight, bheight, cheight, dheight, permanent)
vertex pa;
vertex pb;
vertex pc;
vertex pd;
...
static
    ...
    REAL
    orient3d(
        struct mesh* m, struct behavior* b, vertex pa, vertex pb, vertex pc, vertex pd, REAL aheight, REAL bheight,
        REAL cheight, REAL dheight
    )
#else  /* not ANSI_DECLARATORS */
    REAL
...

{
    if (b->weighted == 0) {
        ...
        return orient3d(
            m, b, pa, pb, pc, pd, pa[0] * pa[0] + pa[1] * pa[1] - pa[2], pb[0] * pb[0] + pb[1] * pb[1] - pb[2],
...
void findcircumcenter(
    struct mesh* m, struct behavior* b, vertex torg, vertex tdest, vertex tapex, vertex circumcenter, REAL* xi,
    REAL* eta, int offcenter
)
#else  /* not ANSI_DECLARATORS */
void findcircumcenter(m, b, torg, tdest, tapex, circumcenter, xi, eta, offcenter) struct mesh* m;
struct behavior* b;
vertex torg;
vertex tdest;
vertex tapex;
...

{
    ...
    while (triangleloop.tri != (triangle*)NULL) {
        ...
        for (triangleloop.orient = 0; triangleloop.orient < 3; triangleloop.orient++) {
            ...
            shouldbedelaunay =
                (oppotri.tri != m->dummytri) && !deadtri(oppotri.tri) && (triangleloop.tri < oppotri.tri) &&
...
#ifdef ANSI_DECLARATORS
void enqueuebadtri(
    struct mesh* m, struct behavior* b, struct otri* enqtri, REAL minedge, vertex enqapex, vertex enqorg, vertex enqdest
)
#else  /* not ANSI_DECLARATORS */
            ...
            if (b->conformdel ||
                (dotproduct * dotproduct >=
                 (2.0 * b->goodangle - 1.0) * (2.0 * b->goodangle - 1.0) *
                     ((eorg[0] - eapex[0]) * (eorg[0] - eapex[0]) + (eorg[1] - eapex[1]) * (eorg[1] - eapex[1])) *
...
#ifdef ANSI_DECLARATORS
enum locateresult
preciselocate(struct mesh* m, struct behavior* b, vertex searchpoint, struct otri* searchtri, int stopatsubsegment)
#else  /* not ANSI_DECLARATORS */
...

{
    ...
    while (totalsamplesleft > 0) {
        ...
        firsttri = (char*)(alignptr + (INT_PTR)m->triangles.alignbytes - (alignptr % (INT_PTR)m->triangles.alignbytes));
...
enum insertvertexresult insertvertex(
    struct mesh* m, struct behavior* b, vertex newvertex, struct otri* searchtri, struct osub* splitseg,
    int segmentflaws, int triflaws
)
#else  /* not ANSI_DECLARATORS */
enum insertvertexresult insertvertex(m, b, newvertex, searchtri, splitseg, segmentflaws, triflaws)
struct mesh* m;
struct behavior* b;
vertex newvertex;
struct otri* searchtri;
...
void triangulatepolygon(
    struct mesh* m, struct behavior* b, struct otri* firstedge, struct otri* lastedge, int edgecount, int doflip,
    int triflaws
)
#else  /* not ANSI_DECLARATORS */
...
#ifdef ANSI_DECLARATORS
void mergehulls(
    struct mesh* m, struct behavior* b, struct otri* farleft, struct otri* innerleft, struct otri* innerright,
...
#ifdef ANSI_DECLARATORS
void divconqrecurse(
    struct mesh* m, struct behavior* b, vertex* sortarray, int vertices, int axis, struct otri* farleft,
    struct otri* farright
)
#else  /* not ANSI_DECLARATORS */
...
struct splaynode* circletopinsert(
    struct mesh* m, struct behavior* b, struct splaynode* splayroot, struct otri* newkey, vertex pa, vertex pb,
    vertex pc, REAL topy
)
#else  /* not ANSI_DECLARATORS */
...
struct splaynode* frontlocate(
    struct mesh* m, struct splaynode* splayroot, struct otri* bottommost, vertex searchvertex, struct otri* searchtri,
    int* farright
)
#else  /* not ANSI_DECLARATORS */
    ...
    while (heapsize > 0) {
        ...
        if (nextevent->xkey < m->xmin) {
            ...
            if (randomnation(SAMPLERATE) == 0) {
                ...
                splayroot =
                    circletopinsert(m, b, splayroot, &lefttri, leftvertex, midvertex, rightvertex, nextevent->ykey);
            }
        } else {
...
#ifdef ANSI_DECLARATORS
int reconstruct(
    struct mesh* m, struct behavior* b, int* trianglelist, REAL* triangleattriblist, REAL* trianglearealist,
    int elements, int corners, int attribs, int* segmentlist, int* segmentmarkerlist, int numberofsegments
)
#else  /* not ANSI_DECLARATORS */
int reconstruct(
    m, b, trianglelist, triangleattriblist, trianglearealist, elements, corners, attribs, segmentlist,
...
#ifdef ANSI_DECLARATORS
long reconstruct(
    struct mesh* m, struct behavior* b, char* elefilename, char* areafilename, char* polyfilename, FILE* polyfile
)
#else  /* not ANSI_DECLARATORS */
...
void segmentintersection(
    struct mesh* m, struct behavior* b, struct otri* splittri, struct osub* splitsubseg, vertex endpoint2
)
#else  /* not ANSI_DECLARATORS */
        ...
        while ((encloop != (struct badsubseg*)NULL) && (m->steinerleft != 0)) {
            ...
            if (!deadsubseg(currentenc.ss) && (eorg == encloop->subsegorg) && (edest == encloop->subsegdest)) {
                ...
                if (!b->conformdel && !acuteorg && !acutedest) {
                    apex(enctri, eapex);
                    while ((vertextype(eapex) == FREEVERTEX) && ((eorg[0] - eapex[0]) * (edest[0] - eapex[0]) +
...
#ifdef ANSI_DECLARATORS
void transfernodes(
    struct mesh* m, struct behavior* b, REAL* pointlist, REAL* pointattriblist, int* pointmarkerlist,
    int numberofpoints, int numberofpointattribs
)
#else  /* not ANSI_DECLARATORS */
    void transfernodes(
        m, b, pointlist, pointattriblist, pointmarkerlist, numberofpoints, numberofpointattribs
    ) struct mesh* m;
    struct behavior* b;
    REAL* pointlist;
...
#ifdef ANSI_DECLARATORS
void readholes(
    struct mesh* m, struct behavior* b, FILE* polyfile, char* polyfilename, REAL** hlist, int* holes, REAL** rlist,
    int* regions
)
#else  /* not ANSI_DECLARATORS */
...

{
    ...
    if (*trianglelist == (int*)NULL) {
        *trianglelist =
            (int*)trimalloc((int)(m->triangles.items * ((b->order + 1) * (b->order + 2) / 2) * sizeof(int)));
    }
    ...
    while (triangleloop.tri != (triangle*)NULL) {
        ...
        if (b->order == 1) {
            ...
            fprintf(
                outfile, "%4ld    %4d  %4d  %4d  %4d  %4d  %4d", elementnumber, vertexmark(p1), vertexmark(p2),
...
#ifdef ANSI_DECLARATORS
void writepoly(
    struct mesh* m, struct behavior* b, char* polyfilename, REAL* holelist, int holes, REAL* regionlist, int regions,
    int argc, char** argv
)
#else  /* not ANSI_DECLARATORS */
...
#ifdef ANSI_DECLARATORS
void writevoronoi(
    struct mesh* m, struct behavior* b, REAL** vpointlist, REAL** vpointattriblist, int** vpointmarkerlist,
    int** vedgelist, int** vedgemarkerlist, REAL** vnormlist
)
#else  /* not ANSI_DECLARATORS */
    void writevoronoi(
        m, b, vpointlist, vpointattriblist, vpointmarkerlist, vedgelist, vedgemarkerlist, vnormlist
    ) struct mesh* m;
    struct behavior* b;
    REAL** vpointlist;
...

{
    ...
    while (triangleloop.tri != (triangle*)NULL) {
        ...
        fprintf(
            outfile, " 3   %4d  %4d  %4d\n", vertexmark(p1) - b->firstnumber, vertexmark(p2) - b->firstnumber,
...

{
...
#ifdef TRILIBRARY
    transfernodes(
        &m, &b, in->pointlist, in->pointattributelist, in->pointmarkerlist, in->numberofpoints,
    ...
    if (b.refine) {
        ...
        m.hullsize = reconstruct(
            &m, &b, in->trianglelist, in->triangleattributelist, in->trianglearealist, in->numberoftriangles,
            in->numberofcorners, in->numberoftriangleattributes, in->segmentlist, in->segmentmarkerlist,
            in->numberofsegments
        );
#else  /* not TRILIBRARY */
...
#ifdef TRILIBRARY
        writevoronoi(
            &m, &b, &vorout->pointlist, &vorout->pointattributelist, &vorout->pointmarkerlist, &vorout->edgelist,
